...
/*    public void setPageMax(String pageMax) {
    ...
    public int doStartTag() throws JspException {
        ...
        if (pageIndex == 1) {
            ...
            str.append(" <a href='" + url + "condition.pageIndex=1'>首页</a> "
                    + "<a href='" + url + "condition.pageIndex=" + (pageIndex - 1) + "'>上一页</a> ");
        }
        if (pageIndex / 6 < 1.0 || pageMax < 10) {
            for (int i = 1; i <= 9; i++) {
                if (i <= pageMax) {
                    if (pageIndex != i) {
                        str.append("<a href='" + url + "condition.pageIndex=" + i + "'>[" + i + "]</a> ");
                    } else {
                        str.append("  " + i + " ");
                    }
                }
        ...
        if (pageIndex == pageMax || pageMax < 2) {
            ...
            str.append("<a href='" + url + "condition.pageIndex=" + (pageIndex + 1) + "'>下一页</a> "
                    + "<a href='" + url + "condition.pageIndex=" + pageMax + "'>尾页</a>");
        }
        str.append("&nbsp;跳转至：<select onchange=\"location='"+url+"condition.pageIndex='+this.value;\">");
